...
* $Rev: 20868 $
* $Author: ntchoumi $
* $URL: http://subversion-01/svn/SW_HW_System/Projects/Ford_SCM/SW/MOD/MOD_POWERMANAGER/trunk/src/Pwrmgr_C1.c $
*-----------------------------------------------------------------------------
*/
/******************************************************************************/
...
SuccessFailType Pwrmgr_Manager(void)
{
    ...
    /* If rate of change of pwrmgrVoltage is greater than X then A2D circuit is   */
    /* broken. Add FAULT handling for RATE of CHANGE                        */
    switch (pwrmgrState)
    {
...
SuccessFailType Pwrmgr_Manager_Handler_For_Rapid_Voltage_Changes(void)
{
    ...
    else if (pwrmgrVoltageForRapidVoltageChanges < (BATTERY_VOLTAGE_EIGENDIAG_LOW))
    {
        _pwrmgrCurrentScopeEigenDiag = FALSE;
    }
